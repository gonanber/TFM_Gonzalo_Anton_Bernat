...
save(eset2, file = "../0_data/eset2_GSE87847.rda")
...
genedata <- genedata[,c("ID","Gene_ID")]
genedata$Symbol <- sub(" ///.*","",genedata$ID)
...
genedata <- genedata[match(rownames(exp3),genedata$Symbol),c("Symbol", "Gene_ID" )]
...
fit2 <- eBayes(fit2)
#save(fit2, file = "../fits/GSE87847.rda")

todos_fit2 <- list()
todos_fit2[["GSE87847"]] <- fit2
