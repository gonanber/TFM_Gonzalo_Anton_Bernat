...
load("../0_data/GSE18123_2.rda")
class(GSE18123)
dim(exprs(GSE18123))

# Los datos no están normalizados con log

datos <- pData(GSE18123)
exp <- exprs(GSE18123)
min(exp)
max(exp)
...
data("y.probes")
sondas_y <- y.probes$affy_hugene_1_0_st_v1
...
exprs(GSE18123) <- exp
massi_y_out <- massi_y(expression_data=GSE18123, y_probes=sondas_y)
...
massi_select_out <- massi_select(expression_data=GSE18123, y_probes=sondas_y, threshold=4)
...
prueba <- data.frame(datos$characteristics_ch1.1, datos$sexo_imputado)

prueba$genero <- sub(".*: ", "", prueba$datos.characteristics_ch1.1)
...
datos$sex_diagnosis <- ifelse(
  grepl("female", datos$`characteristics_ch1.1`, fixed = TRUE) & grepl("diagnosis: AUTISM", datos$`characteristics_ch1`, fixed = TRUE), "female_ASD",
  ifelse(
    grepl("female", datos$`characteristics_ch1.1`, fixed = TRUE) & grepl("diagnosis: ASPERGER'S DISORDER", datos$`characteristics_ch1`, fixed = TRUE), "female_asperger",
    ifelse(
      grepl("female", datos$`characteristics_ch1.1`, fixed = TRUE) & grepl("diagnosis: CONTROL", datos$`characteristics_ch1`, fixed = TRUE), "female_control",
      ifelse(
        grepl("male", datos$`characteristics_ch1.1`, fixed = TRUE) & grepl("diagnosis: AUTISM", datos$`characteristics_ch1`, fixed = TRUE), "male_ASD",
        ifelse(
          grepl("male", datos$`characteristics_ch1.1`, fixed = TRUE) & grepl("diagnosis: ASPERGER'S DISORDER", datos$`characteristics_ch1`, fixed = TRUE), "male_asperger",
          ifelse(
            grepl("male", datos$`characteristics_ch1.1`, fixed = TRUE) & grepl("diagnosis: CONTROL", datos$`characteristics_ch1`, fixed = TRUE), "male_control",
            NA
          )
        )
      )
...
datos <- subset(datos, sex_diagnosis != "male_asperger" & sex_diagnosis != "female_asperger")
...
result <- result$data
datos_x <- subset(result, x >200)
datos_y <- subset(result, y < -50)
...
info <- experimentData(GSE18123)
anotacion <- fData(GSE18123)
anotacion_filtrada <- AnnotatedDataFrame(anotacion[rownames(fData(GSE18123)) %in% rownames(exp2), ])
...
save(eset2, file = "../0_data/eset2_GSE18123_2.rda")
...
load("../0_data/eset2_GSE18123_2.rda")
exp <- exprs(eset2)
datos <- pData(eset2)
info <- experimentData(eset2)

genedata <- fData(eset2)
genes <- as.data.frame(genedata$gene_assignment)
colnames(genes) <- c("genes_raros")
genes$genes_raros <- as.character(genes$genes_raros)

# Carga el paquete tidyr
library(tidyr)

# Suponiendo que tu dataframe se llama "genes" y la columna "genes_raros"

# Separar la columna genes_raros en dos columnas usando "//" como separador
genes_buenos <- separate(genes, genes_raros, into = c("ID", "Symbol"), sep = "//")
genes_buenos$ID <- seq(1, length(genes_buenos$ID))

genedata <- genes_buenos[,c("Symbol","ID")]
...
fit2 <- eBayes(fit2)
#save(fit2, file = "../fits/GSE18123_2.rda")

todos_fit2 <- list()
todos_fit2[["GSE18123_2"]] <- fit2
