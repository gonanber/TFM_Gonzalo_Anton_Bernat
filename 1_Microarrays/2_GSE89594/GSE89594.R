...
save(eset2, file = "../0_data/eset2_GSE89594.rda")

###############################################################################
# ANALISIS DE EXPRESION DIFERENCIAL
###############################################################################
load("../0_data/eset2_GSE89594.rda")
exp <- exprs(eset2)
datos <- pData(eset2)
info <- experimentData(eset2)

genedata <- fData(eset2)
genedata <- genedata[,c("anot.GENE_SYMBOL","anot.ID")]
genedata$Symbol <- sub(" ///.*","",genedata$anot.GENE_SYMBOL)
...
genedata <- genedata[match(rownames(exp3),genedata$Symbol),c("Symbol", "anot.ID" )]
...
fit2 <- eBayes(fit2)
#save(fit2, file = "../fits/GSE89594.rda")

todos_fit2 <- list()
todos_fit2[["GSE89594"]] <- fit2
